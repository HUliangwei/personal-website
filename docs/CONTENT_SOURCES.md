...
| LeRobot / ACT | Public GitHub repo `HUliangwei/robot` (README, `workspace/embodied_learning/`, rollout outputs, `PROGRESS.md`, `note/`), plus the earlier private embodied-AI resume checklist | Public Repository; private resume notes remain TODO | PushT reproduction claims (dataset → ACT training → dual-environment inference, official coverage 0.9534 reproduced, physics-fidelity fix +104% reward) may be published with the repository as the source. LIBERO / SmolVLA / OpenVLA stay **Learning Topics / In Progress** until their sub-project artifacts are complete. Do not claim training or inference success beyond what the repository records. |
...
| Robot / embodied learning (LeRobot, MuJoCo, ACT, PushT reproduction metrics) | Public GitHub repo `HUliangwei/robot` (README, rollout outputs, PROGRESS) | Public Repository | Publish the reproduced coverage figures and physics-fix result exactly as the repository records them; LIBERO / VLA remain in-progress topics. |
