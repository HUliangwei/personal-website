const zh = {
  localeName: '中文',
  alternateLocaleName: 'English',
  htmlLang: 'zh-CN',
  ogLocale: 'zh_CN',
  siteName: '胡良玮',
  siteDescription: '胡良玮的研究与工程作品集，关注智能硬件、集成电路、机器人与具身智能。',
  skipLink: '跳至主要内容',
  navigation: {
    label: '主导航',
    open: '打开导航菜单',
    close: '关闭导航菜单',
    toggle: '切换导航菜单',
    languageSwitch: '切换至 English',
    items: [
      { path: '/', label: '首页' },
      { path: '/about', label: '关于' },
      { path: '/projects', label: '项目' },
      { path: '/cv', label: '简历' },
    ],
  },
  footer: {
    label: '页脚导航',
    cv: '简历',
    emailPending: '邮箱：待核实',
  },
  meta: {
    home: {
      title: '胡良玮 | 研究与工程',
      description: '一份连接智能硬件、集成电路、计算、机器人与自主系统的研究与工程作品集。',
    },
    about: {
      title: '关于 | 胡良玮',
      description: '从物理系统与智能硬件出发，逐步走向机器人、具身智能与自主系统的技术路径。',
    },
    projects: {
      title: '项目 | 胡良玮',
      description: '集成电路、机器人与具身智能方向的研究和工程案例，未核实的细节保持明确标记。',
    },
    cv: {
      title: '简历 | 胡良玮',
      description: '集成电路与具身智能方向的可预览简历，以及内容尚待核实的量子计算方向。',
    },
  },
  home: {
    hero: {
      eyebrow: '胡良玮',
      title: '研究与工程',
      summary: '关注智能硬件、计算与自主系统如何从底层约束走向完整实现。',
      disciplines: ['集成电路', '机器人', '具身智能'],
      projectsAction: '浏览项目',
      cvAction: '查看简历',
      diagramTitle: '从器件到智能系统的概念图',
      diagramDescription: '概念流程从器件和传感器输入开始，经过信号与集成电路计算阶段，最终连接智能系统或机器人。',
      diagramLabels: ['器件 / 传感器', '信号', '集成电路 / 计算', '智能系统 / 机器人'],
    },
    focus: {
      eyebrow: '01 / 方向',
      title: '当前关注',
      description: '持续发展并以证据记录的研究方向。',
      areas: ['集成电路设计', '机器人与具身智能', '智能硬件', '量子与新兴计算'],
    },
    projects: {
      eyebrow: '02 / 项目',
      title: '精选项目',
      all: '全部项目',
    },
    capabilities: {
      eyebrow: '03 / 实践',
      title: '能力版图',
      description: '以下技术领域仍会随着可核实经历继续补充。',
      items: [
        ['硬件与电路', '待核实：补充可公开的方法与工具。'],
        ['传感与信号', '待核实：补充可公开的方法与工具。'],
        ['系统与机器人', '待核实：补充可公开的方法与工具。'],
        ['研究实践', '待核实：补充可公开的方法与工具。'],
      ],
    },
    timeline: {
      eyebrow: '04 / 路径',
      title: '技术轨迹',
      stages: [
        ['基础', '器件、信号与集成电路。'],
        ['系统', '传感、计算与自主系统。'],
        ['下一步', '待核实：补充可公开的里程碑与日期。'],
      ],
    },
    contact: {
      eyebrow: '05 / 联系',
      title: '继续交流',
      description: '联系方式正在整理中。待核实：补充本人确认的首选联系方式。',
      action: '查看简历',
    },
  },
  about: {
    hero: {
      eyebrow: '关于',
      title: '一条仍在展开的技术路径。',
      summary: '我关注物理系统、计算，以及机器感知世界并采取行动的方式如何彼此连接。',
      visualLabel: '从硬件通向具身系统的概念路径',
      visualSteps: ['硬件', '信号', '系统', '行动'],
    },
    statement: {
      eyebrow: '工作方式',
      title: '沿着一个问题，追踪它穿过不同技术层次。',
      body: '这可能从电子或传感问题开始，继续追问信号如何处理、系统如何利用信号，以及当系统必须在物理世界中运行时会发生什么变化。这是一条发展中的方向，而非写定的个人传记；具体内容会来自后续可核实的项目记录。',
    },
    journey: {
      eyebrow: '发展中的路径',
      title: '我的技术旅程',
      intro: '这里不是职位列表，而是逐步把我带到下一层技术的问题。',
      stages: [
        ['电子与硬件', '从物理约束、信号和技术系统的基本构件出发。'],
        ['SPAD 与集成电路', '关注物理事件如何成为可用信号，以及相关传感和读出电路问题。'],
        ['机器人', '转向结合感知、计算和环境交互的完整系统。'],
        ['具身智能', '继续学习当智能必须面对机器人、环境与反馈约束时的学习和控制。'],
      ],
      pendingTitle: '量子与新兴计算',
      pendingBody: '待核实：仅在真实学习或项目背景得到确认后补充。',
    },
    education: {
      eyebrow: '背景',
      title: '教育经历',
      body: '正式教育为这条技术路径提供背景；这里暂不把它写成简历条目。',
      pending: '待核实：后续页面将使用已核实的学校、项目与就读时间。',
    },
    interests: {
      eyebrow: '值得持续追问的问题',
      title: '研究兴趣',
      items: [
        ['智能硬件', '传感、信号、电路与计算之间的关系。'],
        ['机器人', '让感知与行动在物理环境中协同工作的系统。'],
        ['具身智能', '需要同时考虑机器人、环境与反馈的学习和控制。'],
      ],
    },
    connection: {
      eyebrow: '一个连贯视角',
      title: '这些兴趣如何连接',
      visualLabel: '物理系统通向传感与信号，再通向计算和具身行动。',
      steps: ['物理系统', '传感与信号', '计算', '具身行动'],
      body: '这些领域彼此不同，却汇合于同一个实践问题：技术系统如何在尊重底层约束的前提下感知、决策并行动？',
    },
    profile: {
      eyebrow: '跨层实践',
      title: '技术侧写',
      items: [
        ['硬件与信号', '从器件和信号行为出发，思考可实现的系统。'],
        ['系统构建', '连接不同接口的组件，而不是把它们当作孤立工具。'],
        ['研究实践', '通过实验、迭代和清晰记录，让问题可以被检验。'],
      ],
    },
    focus: {
      eyebrow: '现在',
      title: '当前关注',
      body: '我在继续连接集成电路、机器人与具身智能方向，同时把下一步具体问题留到能够用证据准确描述时再公开。',
      pending: '待核实：补充当前研究重点的本人确认表述。',
    },
    outside: {
      eyebrow: '视角',
      title: '研究之外',
      body: '优秀工程也来自对单一工具或学科之外的好奇。这里暂时保持简洁，直到个人兴趣能够用本人确认的文字表达。',
      pending: '待核实：如有必要，后续补充个人说明。',
    },
  },
  projects: {
    intro: {
      eyebrow: '项目',
      title: '工程与研究项目',
      description: '项目细节在得到证据支持之前，会明确标记为待核实。',
    },
    filterLabel: '按类别筛选项目',
    allCategory: '全部',
    viewProject: '查看项目',
    technologies: '技术',
    coverPending: '待补充项目封面图',
    coverAltSuffix: '封面',
    categories: {
      'Integrated Circuits': '集成电路',
      Robotics: '机器人',
      'Embodied AI': '具身智能',
      Quantum: '量子技术',
      Software: '软件',
    },
    entries: {
      spad: { title: '1×16 通道 SPAD 单光子探测器读出芯片', summary: '面向 SPAD 单光子探测的 1×16 通道混合信号读出芯片设计，当前处于版图后仿真与流片前阶段。' },
      lerobot: { title: 'LeRobot / ACT 学习项目', summary: '围绕机器人学习工作流建立的在研学习项目；尚无产物佐证的环节均明确标记待核实。' },
      'mobile-robot': { title: '基于 ROS 的移动机器人', summary: '本科阶段完成的移动机器人项目，连接 Python / YOLO 视觉、ROS、MCU 电机控制与任务闭环。' },
      'quantum-hfss': { title: '超导量子芯片 HFSS 电磁仿真', summary: '百度实习期间开展的超导量子芯片相关微波结构 HFSS 三维电磁仿真与几何优化工作。' },
    },
    detail: {
      back: '返回全部项目',
      date: '日期',
      status: '状态',
      role: '角色',
      technologies: '技术',
      links: '相关链接',
      linksPending: '待核实：补充可公开的项目链接。',
      coverPending: '项目封面待补充',
      descriptionPending: '项目案例细节尚待核实。',
      descriptionSuffix: '未核实内容不会作为事实发布。',
    },
  },
  cv: {
    eyebrow: '简历',
    title: '面向不同技术方向的简历。',
    intro: '选择一个方向，可预览、打开或下载对应 PDF。',
    tracks: [
      {
        title: '集成电路',
        description: '面向集成电路设计及相关工程工作的简历。',
        pdf: '/cv/liangwei-hu-ic-design.pdf',
        available: true,
      },
      {
        title: '具身智能',
        description: '面向机器人、具身智能与 AI 系统工作的简历。',
        pdf: '/cv/liangwei-hu-embodied-ai.pdf',
        available: true,
      },
      {
        title: '量子计算',
        description: '量子计算方向简历将在内容得到核实后发布。',
        pdf: '',
        available: false,
      },
    ],
    actions: {
      groupSuffix: 'PDF 操作',
      preview: '预览 PDF',
      hidePreview: '收起预览',
      open: '打开 PDF',
      download: '下载 PDF',
      fallbackPrefix: '浏览器无法内嵌显示此 PDF。请改为',
      fallbackLink: '打开 PDF',
      comingSoon: '即将提供——待核实的量子计算简历 PDF 发布后会在此补充。',
    },
  },
} as const;

export default zh;
